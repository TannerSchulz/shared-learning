...
{
    public class ServerUserAccountService(SignInManager<ApplicationUser> _signInManager, UserManager<ApplicationUser> _userManager, IConfiguration _configuration, UnitOfWork _unitOfWork) : IUserAccountService
    {
        public async Task<Status> Register(RegisterModel model)
        {
            ...
            var p = new FLP.Data.Entities.Profile()
            {
                ApplicationUser = user,
                DisplayName = user.UserName
            };
            _unitOfWork.Profile.Insert(p);
            _unitOfWork.Save();
